...
export const debugLog = (message: string): void => {
  if (IS_STANDALONE) {
    try {
      const logPath = path.join(path.dirname(process.execPath), 'agent-ironman-debug.log');
...
export const getBinaryDir = (): string => {
  // import.meta.dir gives us /path/to/agent-ironman/server
  // We need to go up one level to /path/to/agent-ironman (the app root)
